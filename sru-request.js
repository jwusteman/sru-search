 ...
 */
class SruRequest extends  mixinBehaviors([IronA11yKeysBehavior], PolymerElement) {
  static get template() {
    return html`
      <style>
        :host {
        ...
        <div  class="horizontal-holder">
            <iron-icon icon="[[icon]]" class="icon"></iron-icon> <iron-input bind-value="{{query}}">
  ...
  static get properties() {
    ...
    _search () {
        ...
        // `http://www.worldcat.org/webservices/catalog/search/worldcat/sru?query=srw.kw+all+"{$this.query}"&&recordSchema=dc&wskey={built-in-api-key}`
